...
renderSyncSystem      — syncs VisualRenderer to binding (mesh creation, kind transitions)
...
```ts
VisualRenderer = tagged("kind", {
  mesh: { kind: "mesh", geometry: "box"|"sphere"|"plane", color: [r,g,b,a] },
  model: { kind: "model", asset: AssetRef },
})
// AssetRef = { type, uri, sub?, options? }
```

Tagged union discriminated by `kind`. Only the `model` variant carries an AssetRef; `mesh` is purely local. `asset.type` selects resolve behavior.
...
Factory: `createAssetRequestSystem(assetManager, slots, registry)`

### Schema discovery
- At creation, walks each component schema to find fields marked `schema.meta?.kind === "assetRef"`.
- Recurses into `object`, `optional`, and `taggedUnion` (deduplicates paths across variants). Does not recurse into `array`, `tuple`, `map`.
...
| Tagged-union variant switches away from asset-bearing (e.g. `model → mesh`) | Treated as if the asset ref became empty: ref released, slot removed. |
| Tagged-union variant switches to asset-bearing (e.g. `mesh → model`) | Treated as fresh add for the new variant's asset ref (no prior slot existed). |
...
| VisualRenderer removed or kind switched away from model | Binding cleared |
...
Factory: `createRenderSyncSystem(binding)`

Dispatches on `VisualRenderer.kind`:
- **Added, kind=mesh**: Create mesh object (geometry + material), add to scene via binding.
- **Added, kind=model**: Skip (model resolve handles async load).
- **Removed**: Delete binding (regardless of kind).
- **Updated, kind=mesh**: If existing binding is a Mesh, update material/geometry in place. Otherwise (was model or nothing), delete old binding and create fresh mesh.
- **Updated, kind=model**: If existing binding is a Mesh (was mesh kind), delete it. Model resolve will handle the async load.
...
### Basic async load
```
Frame N:   VisualRenderer(model) added → request issued, pending
  ...      loader resolves
...
```
Frame N:   VisualRenderer(model) added (asset already cached)
...
### Kind switch (mesh → model)
```
Frame N:   entity has VisualRenderer(mesh) bound
Frame N+1: update to VisualRenderer(model, uri=X)
           → renderSyncSystem deletes mesh binding
           → assetRequestSystem issues request, slot pending
  ...      X loads
Frame N+K: resolve instantiates clone → active
```
...
```
entity has DirectionalLight (light map) + VisualRenderer(model) (ThreeBinding)
...
**Lifecycle:**
- Remove VisualRenderer → binding cleared, ref released
...
- Kind transition within same frame: ordered writes, end-of-frame has one bound object
...
**Kind transitions:**
- Kind switch mesh→model: renderSyncSystem clears mesh binding, model resolve handles async load
- Kind switch model→mesh: slot cleaned up, renderSyncSystem creates mesh
- Light + VisualRenderer: independent, both present
